Feature: Appointment - Create, Update, Get by Id, Get All, Cancel, Delete
  ...
  Scenario: Creating an Appointment
    ...
    * def appointmentResponse = call read('AppointmentSteps.feature@CreateAppointment') ({ doctorId: doctorId, patientId: patientId })
    And match appointmentResponse.response.id == '#notnull'
    * def appointmentId = appointmentResponse.response.id
    * call read('AppointmentSteps.feature@DeleteAppointment') ({ patientId: patientId, id: appointmentId })
  ...
  Scenario: Updating an Appointment
    ...
    * def updateRequest = ({ doctorId: doctorId, startTime: newStartTime, type: 'CONSULTATION', title: 'Updated Consultation', description: 'Rescheduled appointment for follow-up.' })
  ...
  Scenario: Getting an Appointment
    ...
    * def appointmentResponse = call read('AppointmentSteps.feature@CreateAppointment') ({ doctorId: doctorId, patientId: patientId })
    And match appointmentResponse.response.id == '#notnull'
    * def appointmentId = appointmentResponse.response.id
    * def appointmentGetResponse = call read('AppointmentSteps.feature@GetAppointmentById') ({ patientId: patientId, id: appointmentId })
    * call read('AppointmentSteps.feature@DeleteAppointment') ({ patientId: patientId, id: appointmentId })
    * call read('DoctorSteps.feature@DeleteDoctor') ({ id: doctorId })
    * call read('PatientSteps.feature@DeletePatient') ({ id: patientId })
  ...
  Scenario: Deleting an Appointment
    ...
    * def appointmentId = appointmentResponse.response.id
    * call read('AppointmentSteps.feature@DeleteAppointment') ({ patientId: patientId, id: appointmentId })
    * def appointmentGetResponse = call read('AppointmentSteps.feature@GetAppointmentByIdNotFound') ({ patientId: patientId, id: appointmentId })
    * call read('DoctorSteps.feature@DeleteDoctor') ({ id: doctorId })
    * call read('PatientSteps.feature@DeletePatient') ({ id: patientId })
